...
//        Path rootFolderDir = Path.of(scanner.nextLine());
        ...
        System.out.println("List newFolders in Drive : ");
        System.out.println(listNewSheets);

        for(DriveObjectModun driveObjectModun : listFolderUpSheet.values()){
            if(driveObjectModun.getDuration() == null){
        ...
        for (SheetObjectModun s : listOldSheets) {
            System.out.printf(
                    "|%-5d|%-50s|%-82s|%-10s|%-15s|%n",
                    countOld,
                    truncateDisplay(s.getName(), 50),
        ...
        System.out.println("Sheet Update : ");
        // In tiêu đề
        System.out.println("\n\n\n");
        System.out.println(line);
        System.out.printf("|%-5s|%-50s|%-82s|%-10s|%-15s|%n",
